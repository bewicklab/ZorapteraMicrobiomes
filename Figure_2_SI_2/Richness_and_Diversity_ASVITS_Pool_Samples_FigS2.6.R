...
tree_file<-read.tree('ITS/rooted_tree_out/tree.nwk')
...
targets<-sample_names(rZOTUsingle)[which(rowSums(otu_table(rZOTUsingle)) == 62458)]
rZOTUsingle<-prune_samples(targets,rZOTUsingle)
#Rarefy the merged two sample samples to the one sample read depth
rZOTUsingle<-rarefy_even_depth(rZOTUsingle,rngseed=1,sample.size = 31229)

#Rarefy the ASV table for the environment samples
rZOTUenviro<-rarefy_even_depth(ZNC_physeq_enviro,rngseed=1,sample.size=31229)

#Rarefy the ASV table for the environment samples
rZOTUall<-rarefy_even_depth(ZNC_physeq_adult,rngseed=1,sample.size=31229)
...
p_richness<-p_richness+annotate("text", size=6,x=3, y=50, label= "Kruskal-Wallis")
p_richness<-p_richness+annotate("text", size=6,x=3, y=40, label= paste("p = ",round(kw_richness$p.value,4)))
...
#Add the p-value for the Kruskal-Wallis test somewhere on your figure (you may have to change the x and y positions of this label)
p_shannon<-p_shannon+annotate("text", size=6,x=3, y=1, label= "Kruskal-Wallis")
p_shannon<-p_shannon+annotate("text", size=6,x=3, y=0.75, label= paste("p = ",round(kw_shannon$p.value,6)))
  ...
  for (k in 1:length(rownames(pw_shannon$p.value))){
    for (j in 1:k){
      if (rownames(pw_shannon$p.value)[k]!=colnames(pw_shannon$p.value)[j]){
        #If there is a significant difference or you want to also show non-significant differences...
        if (pw_shannon$p.value[k,j]<0.055 || p_value_ns=='yes'){
          #Add an entry to your tibble include the names of the two groups being compared and the p-value for the comparison
...
p_raos<-p_raos+annotate("text", size=6,x=2, y=0.1, label= paste("p = ",round(kw_raos$p.value,6)))
  ...
  for (k in 1:length(rownames(pw_raos$p.value))){
    for (j in 1:k){
      if (rownames(pw_raos$p.value)[k]!=colnames(pw_raos$p.value)[j]){
        #If there is a significant difference or you want to also show non-significant differences...
        if (pw_raos$p.value[k,j]<0.055 || p_value_ns=='yes'){
          #Add an entry to your tibble include the names of the two groups being compared and the p-value for the comparison
